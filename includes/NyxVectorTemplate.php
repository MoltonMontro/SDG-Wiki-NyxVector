<?php
/**
 * NyxVector - Modern version of MonoBook with fresh look and many usability
 * improvements.
 *
...
/**
 * QuickTemplate subclass for NyxVector
 * @ingroup Skins
 */
class NyxVectorTemplate extends BaseTemplate {
	/* Functions */
	...
	public function execute() {
		...
		if ( $this->config->get( 'NyxVectorUseIconWatch' ) ) {
		...
		<?php Hooks::run( 'NyxVectorBeforeFooter' ); ?>
...
<?php
		...
		foreach ( $portals as $name => $content ) {
			...
			switch ( $name ) {
				...
				case 'TOOLBOX':
					...
					Hooks::run( 'NyxVectorAfterToolbox' );
					break;
				case 'LANGUAGES':
	...
	protected function renderNavigation( array $elements ) {
		...
		foreach ( $elements as $name => $element ) {
			switch ( $element ) {
				case 'NAMESPACES':
					?>
					<div id="p-namespaces" role="navigation" class="nyxvectorTabs<?php
				...
				case 'VARIANTS':
					?>
					<div id="p-variants" role="navigation" class="nyxvectorMenu<?php
					...
					?>" aria-labelledby="p-variants-label">
						...
						?>
						<input type="checkbox" class="nyxvectorMenuCheckbox" aria-labelledby="p-variants-label" />
				...
				case 'VIEWS':
					?>
					<div id="p-views" role="navigation" class="nyxvectorTabs<?php
				...
				case 'ACTIONS':
					?>
					<div id="p-cactions" role="navigation" class="nyxvectorMenu<?php
					if ( count( $this->data['action_urls'] ) == 0 ) {
						echo ' emptyPortlet';
					}
					?>" aria-labelledby="p-cactions-label">
						<input type="checkbox" class="nyxvectorMenuCheckbox" aria-labelledby="p-cactions-label" />
						<h3 id="p-cactions-label"><span><?php
							$this->msg( 'nyxvector-more-actions' )
						?></span></h3>
						<div class="menu">
							<ul<?php $this->html( 'userlangattributes' ) ?>>
								<?php
				...
				case 'SEARCH':
					...
					<div id="p-search" role="search">
						...
						<form action="<?php $this->text( 'wgScript' ) ?>" id="searchform">
							<div<?php echo $this->config->get( 'NyxVectorUseSimpleSearch' ) ? ' id="simpleSearch"' : '' ?>>
								<?php
	...
	public function makeLink( $key, $item, $options = [] ) {
		...
		if ( isset( $options['nyxvector-wrap'] ) && $options['nyxvector-wrap'] ) {
	...
	public function makeListItem( $key, $item, $options = [] ) {
		...
		if (
			$this->config->get( 'NyxVectorUseIconWatch' )
		...
		if (
			isset( $options['nyxvector-collapsible'] ) && $options['nyxvector-collapsible'] ) {
